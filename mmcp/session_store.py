...
class SessionStore:
    ...
    def record_usage(self, event: UsageEvent) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            ''', (
                ts, event.session_id, event.host_name, event.agent_name,
                event.provider_name, event.model_name, event.input_tokens,
                event.output_tokens, event.cached_input_tokens, event.uncached_input_tokens,
                event.effective_saved_tokens, event.tool_name, event.latency_ms
            ))
            conn.commit()
    ...
    def get_weekly_usage(self) -> dict[str, dict[str, int]]:
        ...
        Returns: { "model_name": { "used": X, "saved": Y } }
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                SELECT 
                    model_name,
                    SUM(input_tokens + output_tokens) as total_used,
